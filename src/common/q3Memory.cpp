...
q3Stack::q3Stack()
    : m_memory(0),
      m_entries((q3StackEntry *)q3Alloc(sizeof(q3StackEntry) * 64)), m_index(0),
      m_allocation(0), m_entryCount(0), m_entryCapacity(64), m_stackSize(0)
{
}
...
void *q3Stack::Allocate(i32 size)
{
    ...
    if (m_entryCount == m_entryCapacity)
    {
        ...
        m_entryCapacity *= 2;
        m_entries =
            (q3StackEntry *)q3Alloc(m_entryCapacity * sizeof(q3StackEntry));
...
q3Heap::q3Heap()
{
    ...
    m_freeBlocks =
        (q3FreeBlock *)q3Alloc(sizeof(q3FreeBlock) * q3k_heapInitialCapacity);
    m_freeBlockCount    = 1;
...
void q3Heap::Free(void *memory)
{
    ...
    if (!merged)
    {
        ...
        if (m_freeBlockCount == m_freeBlockCapacity)
        {
            ...
            m_freeBlocks = (q3FreeBlock *)q3Alloc(sizeof(q3FreeBlock) *
                                                  m_freeBlockCapacity);
...
//------------------------------------------------------------------------------
q3PagedAllocator::~q3PagedAllocator() { Clear(); }
...
void *q3PagedAllocator::Allocate()
{
    ...
    {
        q3Page *page =
            (q3Page *)q3Alloc(m_blockSize * m_blocksPerPage + sizeof(q3Page));
        ++m_pageCount;
        ...
        q3Block *last =
            Q3_PTR_ADD(page->data, m_blockSize * (blocksPerPageMinusOne));
        last->next = NULL;
...
void q3PagedAllocator::Free(void *data)
{
    ...
    {
        if (data >= page->data &&
            data < Q3_PTR_ADD(page->data, m_blockSize * m_blocksPerPage))
        {
            found = true;
            break;
        }
