...
inline void
q3ComputeBasis(const q3Vec3 &a, q3Vec3 *__restrict b, q3Vec3 *__restrict c)
{
    // Suppose vector a has all equal components and is a unit vector: a = (s,
    // s, s) Then 3*s*s = 1, s = sqrt(1/3) = 0.57735027. This means that at
    // least one component of a unit vector must be greater or equal to
    // 0.57735027. Can use SIMD select operation.
