...
//------------------------------------------------------------------------------
void q3ComputeReferenceEdgesAndBasis(const q3Vec3 &eR,
                                     const q3Transform &rtx,
                                     q3Vec3 n,
                                     i32 axis,
                                     u8 *out,
                                     q3Mat3 *basis,
...
//------------------------------------------------------------------------------
void q3ComputeIncidentFace(const q3Transform &itx,
                           const q3Vec3 &e,
                           q3Vec3 n,
                           q3ClipVertex *out)
{
...
i32 q3Clip(const q3Vec3 &rPos,
           const q3Vec3 &e,
           u8 *clipEdges,
           const q3Mat3 &basis,
           q3ClipVertex *incident,
           q3ClipVertex *outVerts,
           r32 *outDepths)
{
    i32 inCount = 4;
    i32 outCount;
    ...
    outCount =
        q3Orthographic(r32(-1.0), e.x, 0, clipEdges[2], in, inCount, out);

    if (!outCount)
        return 0;

    inCount =
        q3Orthographic(r32(-1.0), e.y, 1, clipEdges[3], out, outCount, in);
...
//------------------------------------------------------------------------------
void q3SupportEdge(const q3Transform &tx,
                   const q3Vec3 &e,
                   q3Vec3 n,
                   q3Vec3 *aOut,
                   q3Vec3 *bOut)
...
void q3BoxtoBox(q3Manifold *m, q3Box *a, q3Box *b)
{
    ...
    if (!parallel)
    {
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 6, s, &eMax, q3Vec3(r32(0.0), -C[0][2], C[0][1]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 7, s, &eMax, q3Vec3(r32(0.0), -C[1][2], C[1][1]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 8, s, &eMax, q3Vec3(r32(0.0), -C[2][2], C[2][1]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 9, s, &eMax, q3Vec3(C[0][2], r32(0.0), -C[0][0]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 10, s, &eMax, q3Vec3(C[1][2], r32(0.0), -C[1][0]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 11, s, &eMax, q3Vec3(C[2][2], r32(0.0), -C[2][0]), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 12, s, &eMax, q3Vec3(-C[0][1], C[0][0], r32(0.0)), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 13, s, &eMax, q3Vec3(-C[1][1], C[1][0], r32(0.0)), &nE))
            return;
        ...
        if (q3TrackEdgeAxis(
                &eAxis, 14, s, &eMax, q3Vec3(-C[2][1], C[2][0], r32(0.0)), &nE))
            return;
    }
    ...
    if (axis < 6)
    {
        ...
        q3ComputeReferenceEdgesAndBasis(
            eR, rtx, n, axis, clipEdges, &basis, &e);
        ...
        i32 outNum;
        outNum =
            q3Clip(rtx.position, e, clipEdges, basis, incident, out, depths);
