...
//------------------------------------------------------------------------------
q3DynamicAABBTree::~q3DynamicAABBTree() { q3Free(m_nodes); }
...
void q3DynamicAABBTree::InsertLeaf(i32 id)
{
    ...
    while (!m_nodes[searchIndex].IsLeaf())
    {
        ...
        // descent)
        r32 inheritedCost =
            r32(2.0) * (combinedArea - m_nodes[searchIndex].aabb.SurfaceArea());
...
void q3DynamicAABBTree::SyncHeirarchy(i32 index)
{
    while (index != Node::Null)
    {
        ...
        m_nodes[index].aabb =
            q3Combine(m_nodes[left].aabb, m_nodes[right].aabb);
